...
int main() {
    ...
    if(opt == 2) {
        ...
        if(!strcmp(pword,usr.password)) {
            while(cont == 'y') {
                ...
                switch(choice) {
                    ...
                    break;
                    case 4:
                    printf("\nPlease enter the phone number to transfer the amount of money:\t");
                    scanf("%d" , &phone);
                    ...
                    if(amount > usr.balance) printf("\nInsufficient balance");
                        ...
                        if(fwrite != NULL) {
                            printf("\nYou have successfuly transfered the amount %.2f to %s" , amount , phone);
                            strcpy(filename,usr.phone);
                            fp = fopen(strcat(filename,".dat"),"w");
                            usr.balance -= amount;
                            fwrite(&usr1,sizeof(struct user),1,fp);
        ...
        default:
                ...
                printf("\n do you want to continue the transaction [y\n]");
                scanf("%s" , &cont);
            }
